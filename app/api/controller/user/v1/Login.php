...
class Login extends CheckSignTimes
{
    ...
    public function loginPassword()
    {
        ...
        $key = AdminKeyConfig::find(1)['user'];                                   //获取管理员后台key

        if (filter_var($data['username'], FILTER_VALIDATE_EMAIL)) {
            $res = AdminUsers::where('email', $data['username'])->findOrEmpty();       //通过邮箱查询用户信息
        } elseif (preg_match("/^1[345678]{1}\d{9}$/", $data['username'])) {
            $res = AdminUsers::where('mobile', $data['username'])->findOrEmpty();      //通过手机查询用户信息
        } else {
            $res = AdminUsers::where('username', $data['username'])->findOrEmpty();    //通过用户名查询用户信息
        }
    ...
    public function loginMobile()
    {
        ...
        $res = AdminUsers::where('mobile', $data['mobile'])->findOrEmpty();    //通过用户名查询用户信息
        if ($res->isEmpty()) {
            return $this->returnJson(0, [], '账号或者密码错误', 400);              //账号不存在
        } else {
            //登录成功根据管理后台key下发token
    ...
    public function loginEmail()
    {
        ...
        $key = AdminKeyConfig::find(1)['user'];                                   //获取管理员后台key
        $res = AdminUsers::where('email', $data['email'])->findOrEmpty();    //通过用户名查询用户信息
        if ($res->isEmpty()) {
            return $this->returnJson(0, [], '账号或者密码错误', 400);              //账号不存在
        } else {
            //登录成功根据管理后台key下发token
